...
ディレイのある株価データを悪用したカンニング取引 (表示価格と実際の市場価格の乖離を利用) を防止するため、成行注文に対してスリッページを適用します。

| 項目 | 値 |
|---|---|
| スリッページ率 | 0.05% 〜 0.5% |
| 方向バイアス | 70%で不利方向 (買い→上方向、売り→下方向)、30%で有利方向 |
| 丸め | 日本株: 整数、米国株: 小数2桁 |

### ストップ高/ストップ安 (値幅制限)

日本株は東証の値幅制限テーブル (34段階) に基づき、前日終値からの値幅制限を適用します。

- **ストップ高到達時**: 買い注文をブロック
- **ストップ安到達時**: 売り注文をブロック
- QuoteDisplay にストップ高/安価格とバッジを表示

### S株 (単元未満株) の制約

日本株で100株未満の注文は S株として処理し、以下の制約を適用します:
