...
| 条件 | ルール |
|---|---|
| **成行注文** | スリッページ (±0.05〜0.5%) が適用されます。70%の確率で不利な方向に滑ります |
| **ストップ高/安** | 日本株は東証の値幅制限をチェックします。制限価格に達している場合は注文がブロックされます |
| **S株 (単元未満株)** | 日本株で100株未満の注文。成行注文・現物のみ。前日終値で約定 (SBI証券ルール準拠) |

> S株注文の制約: `type: "LIMIT"` や `tradeType: "MARGIN"` を指定するとエラーになります。
