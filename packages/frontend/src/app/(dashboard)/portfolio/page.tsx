...
export default function PortfolioPage() {
  ...
  return (
    <div className="space-y-4 md:space-y-6 p-3 md:p-6">
      <h1 className="text-xl md:text-2xl font-bold">ポートフォリオ</h1>

      {/* 総資産評価額 */}
      <div className="card bg-gradient-to-r from-gray-900 to-gray-800 border-brand-600/30">
        <div className="flex items-center justify-between mb-3 md:mb-4">
          <p className="text-xs md:text-sm text-gray-400">総資産評価額</p>
          {priceLoading && (
        ...
        <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-6">
          <div>
            ...
            {totalPnlJpy !== 0 && (
              <p className={`text-xs md:text-sm font-mono mt-1 ${totalPnlJpy >= 0 ? "text-red-400" : "text-green-400"}`}>
          ...
          {(totalAssetUsd > 0 || cashUsd > 0) && (
            <div className="border-t sm:border-t-0 sm:border-l border-gray-700 pt-2 sm:pt-0 sm:pl-6">
              <p className="text-2xl md:text-3xl font-bold font-mono">
                ${totalAssetUsd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
              {totalPnlUsd !== 0 && (
                <p className={`text-xs md:text-sm font-mono mt-1 ${totalPnlUsd >= 0 ? "text-green-400" : "text-red-400"}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
        <div className="card">
          <p className="text-xs md:text-sm text-gray-400">現金 (JPY)</p>
          <p className="text-lg md:text-2xl font-bold mt-1 font-mono">
            ¥{cashJpy.toLocaleString()}
          </p>
        </div>
        <div className="card">
          <p className="text-xs md:text-sm text-gray-400">現金 (USD)</p>
          <p className="text-lg md:text-2xl font-bold mt-1 font-mono">
            ${cashUsd.toLocaleString()}
          </p>
        </div>
        <div className="card">
          <p className="text-xs md:text-sm text-gray-400">株式評価額</p>
          <p className="text-lg md:text-2xl font-bold mt-1 font-mono">
        ...
        <div className="card">
          <p className="text-xs md:text-sm text-gray-400">信用使用額</p>
          <p className="text-lg md:text-2xl font-bold mt-1 text-yellow-400 font-mono">
      ...
      {/* 保有銘柄 */}
      <div className="card">
        <h2 className="font-bold text-base md:text-lg mb-3 md:mb-4">保有銘柄</h2>
        {holdings.length === 0 ? (
          <p className="text-gray-500 text-center py-6 md:py-8 text-sm">保有銘柄はありません</p>
        ) : (
          <>
            {/* Mobile card view */}
            <div className="md:hidden space-y-2">
              {holdings.map((h) => {
                ...
                const pnlColor = h.pnl !== undefined
                  ? (isJP ? (h.pnl >= 0 ? "text-red-400" : "text-green-400") : (h.pnl >= 0 ? "text-green-400" : "text-red-400"))
                  : "text-gray-400";
                return (
                  <div key={h.id} className="bg-gray-800/50 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-bold text-brand-400">{h.symbol}</span>
                        <span className="text-[10px] bg-gray-700 px-1.5 py-0.5 rounded text-gray-400">{isJP ? "東証" : "US"}</span>
                      </div>
                      <span className="text-xs text-gray-400">{h.quantity.toLocaleString()}株</span>
                    </div>
                    <div className="flex items-end justify-between">
                      <div className="text-xs text-gray-400">
                        <p>取得: {currency}{h.avgCost.toLocaleString()}</p>
                        <p>現在: {h.currentPrice !== undefined ? `${currency}${h.currentPrice.toLocaleString()}` : "-"}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-mono font-bold text-sm">{currency}{(h.marketValue || 0).toLocaleString()}</p>
                        {h.pnl !== undefined && (
                          <p className={`text-xs font-mono font-bold ${pnlColor}`}>
                            ...
                            {h.pnlPercent !== undefined && <span className="ml-1">({h.pnlPercent >= 0 ? "+" : ""}{h.pnlPercent.toFixed(2)}%)</span>}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    return (
                      <tr key={h.id} className="border-b border-gray-800/50 hover:bg-gray-800/50">
                        ...
                        <td className="table-cell">
                          <span className="text-xs bg-gray-800 px-2 py-0.5 rounded">{isJP ? "東証" : "US"}</span>
                        </td>
                        <td className="table-cell text-right font-mono">{h.quantity.toLocaleString()}</td>
                        <td className="table-cell text-right font-mono">{currency}{h.avgCost.toLocaleString()}</td>
                        <td className="table-cell text-right font-mono">
                          {h.currentPrice !== undefined ? `${currency}${h.currentPrice.toLocaleString()}` : <span className="text-gray-600">-</span>}
                        </td>
                        <td className="table-cell text-right font-mono font-bold">{currency}{(h.marketValue || 0).toLocaleString()}</td>
                        <td className={`table-cell text-right font-mono font-bold ${pnlColor}`}>
                          {h.pnl !== undefined ? (
                            <>
                              {h.pnl >= 0 ? "+" : ""}{currency}{h.pnl.toLocaleString(undefined, { maximumFractionDigits: isJP ? 0 : 2 })}
                              <br />
                              <span className="text-xs">({h.pnlPercent !== undefined ? `${h.pnlPercent >= 0 ? "+" : ""}${h.pnlPercent.toFixed(2)}%` : ""})</span>
                            </>
                          ) : "-"}
                        </td>
                      </tr>
